...
- **`stats.py`** — JWT-protected API traffic analytics. `GET /api/stats/summary?days=N`
  ...
  application (`bible-garden`, `lampada`, `ops`, historical `unknown`). Recent
  requests expose `application` and accept an exact `application` filter.
  ...
  `unknown` rows remain readable. `API_KEY` here remains Dashboard-API's own
  read key.
